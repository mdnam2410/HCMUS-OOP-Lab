...
{
    std::size_t firstColon = input.find(':');
    std::size_t secondColon = input.find(':', firstColon + 1);
    if (firstColon == std::string::npos || secondColon == std::string::npos)
        return nullptr;

    try {
        int h = std::stoi(input.substr(0, firstColon));
        int m = std::stoi(input.substr(firstColon + 1, secondColon - firstColon));
        int s = std::stoi(input.substr(secondColon + 1));
